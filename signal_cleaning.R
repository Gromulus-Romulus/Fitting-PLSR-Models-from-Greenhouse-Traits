...
# Extract column names registered from SVC spectrophotometer
...
pdf("./Figures/spectra_plots.pdf")  # Change file name as needed
...
for (i in 1:length(uniq_ids)) {  # Correct iteration
  ...
  p <- ggplot(df, aes(x = wavelength, y = reflectance, color = sig_proc)) +
    geom_line(size = 1.5) +
    ...
    facet_wrap(~sig_proc, labeller = label_both, scales = "free")  # Allow free y-axis ranges
...
saveRDS(signal.matrices, file = "./data/signal.matrices.RDS")
